...
#Input  sequences  
ref_seqs_DNAss <-
  readDNAStringSet("data/Database_Sequences/unique_resolved_Amphibian_Reference_Sequences_16S.fasta")
...
fuzzy_left_join(ref_seqs_DNAss_df, target_taxa_clean, # this chunk gets only new names
 ...
 fuzzy_left_join(ref_seqs_DNAss_df, synonyms_amphibian, #this chunk gets only old names
  ...
  select(seq_name) %>%
  unique() %>%
  write_csv("data/Taxon_List/species-with-fulllength-16s-Taxa_List.csv")

---
title: "Designing CO1 Primers"
author: "Brandon D. Hoenig"
date: "3/23/2022"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)
```

```{r}
library(tidyverse)
library(aphid)
library(Biostrings)
library(taxreturn)
library(ape)
library(DECIPHER)
library(seqinr)
library(fuzzyjoin)
library(PrimerMiner)
library(vegan)
library(reshape2)
library(ShortRead)
```

Sequences were aligned to Xenopus laevis using MAFFT with default arguments. 
```{r}
dereplicated_seqs_CO1_target_taxa_aligned <-
  readDNAStringSet("data/Database_Sequences/dereplicated_seqs_CO1_target_taxa_aligned.fasta") 
```

Remove sequences that were manually determined to not be CO1. 
```{r}
list <- 
  data.frame(name = c(
names(dereplicated_seqs_CO1_target_taxa_aligned) %>%
  str_remove(., pattern = "New[|]Desmognathus_fuscus_.*") %>% #Too many were not CO1 so removed all
  str_remove(., pattern = "New[|]Brachycephalus_nodoterga_120") %>% #CytB
  str_remove(., pattern = "New[|]Craugastor_gollmeri_1616") %>% # tRNA-met
  str_remove(., pattern = "New[|]Ambystoma_maculatum_362") %>% # tRNA-Thr
  str_remove(., pattern = "New[|]Ambystoma_maculatum_363") %>% #tRNA-Thr
  str_remove(., pattern = "New[|]Lithobates_warszewitschii_117") %>% # NAD1
  str_remove(., pattern = "New[|]Craugastor_noblei_1615") %>% # tRNA-met
  str_remove(., pattern = "New[|]Craugastor_bransfordii_1613") %>% # tRNA-met
  str_remove(., pattern = "New[|]Pristimantis_ridens_1621") %>% #tRNA-met
  str_remove(., pattern = "New[|]Brachycephalus_pitanga_119") %>% #cytB
  str_subset(., pattern = ".+")))

dereplicated_seqs_CO1_target_taxa_aligned <-
  dereplicated_seqs_CO1_target_taxa_aligned[list$name]

writeXStringSet(x = dereplicated_seqs_CO1_target_taxa_aligned, filepath = "data/Database_Sequences/dereplicated_seqs_CO1_target_taxa_aligned_edited.fasta", format = 'fasta')
```

```{r}
dereplicated_seqs_CO1_all_taxa_aligned <-
  readDNAStringSet("data/Database_Sequences/unique_resolved_Amphibian_Reference_Sequences_COI_aligned.fasta") 
```

```{r}

not_any_na <- function(x) all(!is.na(x))

Primer_plot_eachnucletide_eachgenus <-
tibble(
sequences = paste(dereplicated_seqs_CO1_target_taxa_aligned), 
names = names(dereplicated_seqs_CO1_target_taxa_aligned)) %>%
  select(names, sequences) %>% 
  mutate(names = str_remove(names, pattern = "_[^_]*$"),
         names = str_remove(names, pattern = "New[|]")) %>% 
  mutate(site = strsplit(sequences, "")) %>%
        unnest(site) %>%
        group_by(sequences) %>%
        mutate(row = row_number()) %>%
        spread(row, site) %>%
  ungroup()  %>%
  select(-sequences) %>% 
  pivot_longer(cols = 2:ncol(.), 
               names_to = "site", 
               values_to = 'nuc') %>% 
  mutate(site = as.numeric(site)) %>%
  filter(nuc == "A"| 
         nuc == "C"| 
         nuc == "T"|
         nuc == 'G')   %>% 
  group_by(names, site, nuc) %>%
  tally() %>% 
  ungroup() %>% 
  group_by(names, site) %>% 
  mutate(total = sum(n),
         perc = n/total) %>%
  ungroup() %>%
  drop_na(nuc)

ggplot() +
  geom_bar(data = Primer_plot_eachnucletide_eachgenus, 
           aes(x = site, 
               y = perc,
               fill = nuc,
               group = nuc),
           stat = 'identity') +
  geom_text(data = Primer_plot_eachnucletide_eachgenus %>%
              group_by(names) %>%
              slice_head(n = 1),
            aes(y = 0.5, 
                x = -1, 
                hjust = 1,
                label = names)) +
  scale_x_continuous(breaks = seq(1, 652, 1),
                     position = 'top') +
  scale_y_continuous(expand = c(0,0)) +
  coord_cartesian(xlim = c(-5, 652)) +
  theme_void() +
  theme(strip.background = element_blank(),
        strip.text = element_blank(),
        axis.text.x = element_text(colour = 'black',
                                   angle = 90,
                                   hjust = 0.5)) +
  facet_wrap(~names, 
             ncol = 1)
  
ggsave("output/Primer_Plot_eachnucleotide.png", width = 85, height = 50, limitsize = F)
``` 

Plot for all taxa to see best primers
```{r}
Primer_plot_eachnucletide_all <-
tibble(
sequences = paste(dereplicated_seqs_CO1_target_taxa_aligned), 
names = names(dereplicated_seqs_CO1_target_taxa_aligned)) %>%
  select(names, sequences) %>% 
  mutate(names = str_remove(names, pattern = "_[^_]*$"),
         names = str_remove(names, pattern = "New[|]")) %>% 
  mutate(site = strsplit(sequences, "")) %>%
        unnest(site) %>%
        group_by(sequences) %>%
        mutate(row = row_number()) %>%
        spread(row, site) %>%
  ungroup()  %>%
  select(-sequences) %>% 
  pivot_longer(cols = 2:ncol(.), 
               names_to = "site", 
               values_to = 'nuc') %>% 
  mutate(site = as.numeric(site)) %>%
  filter(nuc == "A"| 
         nuc == "C"| 
         nuc == "T"|
         nuc == 'G')   %>% 
  unique() %>%
  group_by(site, nuc) %>%
  tally() %>% 
  ungroup() %>% 
  group_by(site) %>% 
  mutate(total = sum(n),
         perc = n/total) %>%
  ungroup() %>%
  drop_na(nuc) 


ggplot() +
  geom_bar(data = Primer_plot_eachnucletide_all, 
           aes(x = site, 
               y = perc,
               fill = nuc,
               group = -perc),
           stat = 'identity') +
  scale_x_continuous(breaks = seq(1, 652, 1),
                     position = 'top') +
  scale_y_continuous(expand = c(0,0)) +
  coord_cartesian(xlim = c(-5, 652)) +
  theme_void() +
  theme(strip.background = element_blank(),
        strip.text = element_blank(),
        axis.text.x = element_text(colour = 'black',
                                   angle = 90,
                                   hjust = 0.5))

ggsave("output/Primer_Plot_eachnucleotide_all_test.png", width = 85, limitsize = F)
```

#Gotta make the original primer plot to re-evaluate everything.


```{r}
ggplot() +
  geom_bar(data = Primer_plot_eachnucletide_all, 
           aes(x = site, 
               y = perc,
               fill = perc,
               group = -perc),
           stat = 'identity') +
  geom_text(data = Primer_plot_eachnucletide_all %>%
              as.data.frame() %>%
              arrange(site, -perc)  %>%
              group_by(site) %>%
             slice_head(n = 1),
            aes(x = site, 
                y = 1.05, 
                label = nuc,
                colour = perc)) +
  scale_fill_binned(breaks = c(seq(0, 1, 0.20)),
                    type = scale_fill_viridis_c) +
  scale_colour_binned(breaks = c(seq(0, 1, 0.20)),
                    type = scale_colour_viridis_c) +
  scale_x_continuous(breaks = seq(1, 652, 1),
                     position = 'top') +
  scale_y_continuous(expand = c(0,0.075)) +
  coord_cartesian(xlim = c(-5, 652)) +
  theme_void() +
  theme(strip.background = element_blank(),
        strip.text = element_blank(),
        axis.text.x = element_text(colour = 'black',
                                   angle = 90,
                                   hjust = 1))

ggsave("output/Amp_COI_aligned_new_test.png", width = 85, limitsize = F)
```

```{r}
Primer_plot_eachnucletide_all %>%
              as.data.frame() %>%
              arrange(site, -perc)  %>%
              group_by(site) %>%
             slice_head(n = 1) %>% View()
```


PrimerMiner
```{r}
Primer_F1_215_234 <-
PrimerMiner::evaluate_primer(alignment_imp = "data/Database_Sequences/dereplicated_seqs_CO1_target_taxa_aligned_edited.fasta", 
                             primer_sequ = "GCMCCWGAKATRGCHTTYCC",
                             start = 215, 
                             stop = 234) %>% 
  as_tibble()

Primer_F2_248_267 <-
PrimerMiner::evaluate_primer(alignment_imp = "data/Database_Sequences/dereplicated_seqs_CO1_target_taxa_aligned_edited.fasta", 
                             primer_sequ = "ATAAGYTTYTGACTNYTNCC",
                             start = 248, 
                             stop = 267) %>% 
  as_tibble()

Primer_R1_470_489 <-
PrimerMiner::evaluate_primer(alignment_imp = "data/Database_Sequences/dereplicated_seqs_CO1_target_taxa_aligned_edited.fasta", 
                             primer_sequ = "TGRTAYTGDGWTATWGAKGG",
                             start = 470, 
                             stop = 489, forward = F) %>% 
  as_tibble()

Primer_R2_587_606 <-
PrimerMiner::evaluate_primer(alignment_imp = "data/Database_Sequences/dereplicated_seqs_CO1_target_taxa_aligned_edited.fasta", 
                             primer_sequ = "AACGTWGTRTTWARRTTNCG",
                             start = 587, 
                             stop = 606, forward = F) %>% 
  as_tibble()



```



Testing for off-target effects on Homo sapiens... doesn't look good...
```{r}
Primer_F1_215_234_out <-
PrimerMiner::evaluate_primer(alignment_imp = "data/Database_Sequences/Homo_sapiens_CO1_outgroup.fasta", 
                             primer_sequ = "GCMCCWGAKATRGCHTTYCC",
                             start = 215, 
                             stop = 234) %>% 
  as_tibble()

Primer_F2_248_267_out <-
PrimerMiner::evaluate_primer(alignment_imp = "data/Database_Sequences/Homo_sapiens_CO1_outgroup.fasta", 
                             primer_sequ = "ATAAGYTTYTGACTNYTNCC",
                             start = 248, 
                             stop = 267) %>% 
  as_tibble()

Primer_R1_470_489_out <-
PrimerMiner::evaluate_primer(alignment_imp = "data/Database_Sequences/Homo_sapiens_CO1_outgroup.fasta", 
                             primer_sequ = "TGRTAYTGDGWTATWGAKGG",
                             start = 470, 
                             stop = 489, forward = F) %>% 
  as_tibble()

Primer_R2_587_606_out <-
PrimerMiner::evaluate_primer(alignment_imp = "data/Database_Sequences/Homo_sapiens_CO1_outgroup.fasta", 
                             primer_sequ = "AACGTWGTRTTWARRTTNCG",
                             start = 587, 
                             stop = 606, forward = F) %>% 
  as_tibble()
```


```{r}
Primer_F1_215_234 <-
PrimerMiner::evaluate_primer(alignment_imp = "data/Database_Sequences/dereplicated_seqs_CO1_target_taxa_aligned_edited.fasta", 
                             primer_sequ = "GCMCCWGAKATRGCHTTYCC",
                             start = 215, 
                             stop = 234) %>% 
  as_tibble()

Primer_F2_248_267 <-
PrimerMiner::evaluate_primer(alignment_imp = "data/Database_Sequences/dereplicated_seqs_CO1_target_taxa_aligned_edited.fasta", 
                             primer_sequ = "ATAAGYTTYTGACTNYTNCC",
                             start = 248, 
                             stop = 267) %>% 
  as_tibble()

Primer_R1_470_489 <-
PrimerMiner::evaluate_primer(alignment_imp = "data/Database_Sequences/dereplicated_seqs_CO1_target_taxa_aligned_edited.fasta", 
                             primer_sequ = "TGRTAYTGDGWTATWGAKGG",
                             start = 470, 
                             stop = 489, forward = F) %>% 
  as_tibble()

Primer_R2_587_606 <-
PrimerMiner::evaluate_primer(alignment_imp = "data/Database_Sequences/dereplicated_seqs_CO1_target_taxa_aligned_edited.fasta", 
                             primer_sequ = "AACGTWGTRTTWARRTTNCG",
                             start = 587, 
                             stop = 606, forward = F) %>% 
  as_tibble()
```


```{r}
rbind(
Primer_F1_215_234 %>%
  mutate(Template = str_replace(Template, "_[^_]+$", ""),
         Template = str_replace(Template, "New[|]", ""))  %>% 
  filter(., !grepl("-", sequ)) %>%
  mutate(pset = "Primer_F1_215_234"),

Primer_F2_248_267 %>%
  mutate(Template = str_replace(Template, "_[^_]+$", ""),
         Template = str_replace(Template, "New[|]", "")) %>%
  filter(., !grepl("-", sequ))  %>%
  mutate(pset = "Primer_F2_248_267"),

Primer_R1_470_489 %>%
  mutate(Template = str_replace(Template, "_[^_]+$", ""),
         Template = str_replace(Template, "New[|]", "")) %>%
  filter(., !grepl("-", sequ)) %>%
  mutate(pset = "Primer_R1_470_489"),

Primer_R2_587_606 %>%
  mutate(Template = str_replace(Template, "_[^_]+$", ""),
         Template = str_replace(Template, "New[|]", "")) %>%
  filter(., !grepl("-", sequ)) %>%
  mutate(pset = "Primer_R2_587_606")
) %>%
  pivot_longer(cols = V20:V1, 
               names_to = 'site', 
               values_to = 'score') %>%
  mutate(site = as.numeric(str_remove(site, "V"))) %>%
  select(site, score, pset,sum) %>%
  filter(sum > 60) %>%
  ggplot() +
  geom_boxplot(aes(x = -site, 
                   y = score, 
                   group = site),
               fill = 'transparent') +
  facet_wrap(~pset)
```

```{r}
rbind(
Primer_F1_215_234 %>%
  mutate(Template = str_replace(Template, "_[^_]+$", ""),
         Template = str_replace(Template, "New[|]", ""))  %>% 
  filter(., !grepl("-", sequ)) %>%
  mutate(pset = "Primer_F1_215_234"),

Primer_F2_248_267 %>%
  mutate(Template = str_replace(Template, "_[^_]+$", ""),
         Template = str_replace(Template, "New[|]", "")) %>%
  filter(., !grepl("-", sequ))  %>%
  mutate(pset = "Primer_F2_248_267"),

Primer_R1_470_489 %>%
  mutate(Template = str_replace(Template, "_[^_]+$", ""),
         Template = str_replace(Template, "New[|]", "")) %>%
  filter(., !grepl("-", sequ)) %>%
  mutate(pset = "Primer_R1_470_489"),

Primer_R2_587_606 %>%
  mutate(Template = str_replace(Template, "_[^_]+$", ""),
         Template = str_replace(Template, "New[|]", "")) %>%
  filter(., !grepl("-", sequ)) %>%
  mutate(pset = "Primer_R2_587_606")
) %>%
  pivot_longer(cols = V20:V1, 
               names_to = 'site', 
               values_to = 'score') %>%
  mutate(site = as.numeric(str_remove(site, "V"))) %>%
  filter(sum > 60) %>%
  filter(pset == "Primer_R2_587_606" |
         pset == "Primer_F2_247_266") %>% 
  unique() %>% View()
```

```{r}

primer_threshold(fw = "output/Primer_F2_248_267.csv",
                 rw = "output/Primer_R2_587_606.csv",
                 threshold = 120)
```


```{r}
derep_Primer_F1_R1 <-
as.DNAbin(clean(subseq(dereplicated_seqs_CO1_target_taxa_aligned, start = 235, end = 469)))

distF1R1 <-
melt(dist.dna(derep_Primer_F1_R1, model = 'raw', pairwise.deletion = T, as.matrix = T)) %>%
  mutate(pset = 'F1R1',
         value = value * 235)

derep_Primer_F1_R2 <-
as.DNAbin(clean(subseq(dereplicated_seqs_CO1_target_taxa_aligned, start = 235, end = 586)))

distF1R2 <-
melt(dist.dna(derep_Primer_F1_R2, model = 'raw', pairwise.deletion = T, as.matrix = T))%>%
  mutate(pset = 'F1R2',
         value = value * 352)

derep_Primer_F2_R1 <-
as.DNAbin(clean(subseq(dereplicated_seqs_CO1_target_taxa_aligned, start = 268, end = 469)))

distF2R1 <-
melt(dist.dna(derep_Primer_F2_R1, model = 'raw', pairwise.deletion = T, as.matrix = T))%>%
  mutate(pset = 'F2R1',
         value = value * 203)

derep_Primer_F2_R2 <-
as.DNAbin(clean(subseq(dereplicated_seqs_CO1_target_taxa_aligned, start = 268, end = 586)))

distF2R2 <-
melt(dist.dna(derep_Primer_F2_R2, model = 'raw', pairwise.deletion = T, as.matrix = T))%>%
  mutate(pset = 'F2R2',
         value = value * 319)
```

```{r}
distF2R2 %>% 
  filter(Var1 != Var2) %>% 
  mutate(Var1 = str_replace(Var1, "_[^_]+$", ""),
         Var1 = str_replace(Var1, "New[|]", ""),
         Var2 = str_replace(Var2, "_[^_]+$", ""),
         Var2 = str_replace(Var2, "New[|]", "")) %>%
  mutate(variation = if_else(Var1 == Var2, "intra", "inter"))  %>% 
  unique() %>% View()
  ggplot() +
  geom_violin(aes(y = value, 
                   fill = pset,
                   x =  variation))

ggsave("output/specificDifferencesbyampsize.png")
```

```{r}
distF2R2 %>% 
  filter(!grepl("Xenopus", Var1)) %>%
  filter(!grepl("Xenopus", Var2)) %>%
  filter(Var1 != Var2) %>% 
  mutate(Var1 = str_replace(Var1, "_[^_]+$", ""),
         Var1 = str_replace(Var1, "New[|]", ""),
         Var2 = str_replace(Var2, "_[^_]+$", ""),
         Var2 = str_replace(Var2, "New[|]", "")) %>% 
  separate(Var2, into = c("Var2_genus", "Var2_species"), sep = "_", extra = 'merge') %>%
  separate(Var1, into = c("Var1_genus", "Var1_species"), sep = "_", extra = 'merge')

```


```{r}
Primer_F1_215_234_all <-
PrimerMiner::evaluate_primer(alignment_imp = "data/Database_Sequences/unique_resolved_Amphibian_Reference_Sequences_COI_aligned.fasta", 
                             primer_sequ = "GCMCCWGAKATRGCHTTYCC",
                             start = 215, 
                             stop = 234,
                             save = "output/Primer_F1_215_234_all.csv") %>% 
  as_tibble()

Primer_F2_248_267_all <-
PrimerMiner::evaluate_primer(alignment_imp = "data/Database_Sequences/unique_resolved_Amphibian_Reference_Sequences_COI_aligned.fasta", 
                             primer_sequ = "ATAAGYTTYTGACTNYTNCC",
                             start = 248, 
                             stop = 267) %>% 
  as_tibble() # This is the one we're going with. 

Primer_R1_470_489_all <-
PrimerMiner::evaluate_primer(alignment_imp = "data/Database_Sequences/unique_resolved_Amphibian_Reference_Sequences_COI_aligned.fasta", 
                             primer_sequ = "TGRTAYTGDGWTATWGAKGG",
                             start = 470, 
                             stop = 489, forward = F,
                             save = "output/Primer_R1_470_489_all.csv") %>% 
  as_tibble()

Primer_R2_587_606_all <-
PrimerMiner::evaluate_primer(alignment_imp = "data/Database_Sequences/unique_resolved_Amphibian_Reference_Sequences_COI_aligned.fasta", 
                             primer_sequ = "AACGTWGTRTTWARRTTNCG",
                             start = 587, 
                             stop = 606, forward = F) %>% 
  as_tibble() # this is the one we're going with.
```

```{r}
rbind(
Primer_F2_248_267_all %>%
  mutate(Template = str_replace(Template, "_[^_]+$", ""),
         Template = str_replace(Template, "New[|]", "")) %>%
  filter(., !grepl("-", sequ))  %>%
  mutate(pset = "Primer_F2_248_267"),

Primer_R2_587_606_all %>%
  mutate(Template = str_replace(Template, "_[^_]+$", ""),
         Template = str_replace(Template, "New[|]", "")) %>%
  filter(., !grepl("-", sequ)) %>%
  mutate(pset = "Primer_R2_587_606")
) %>%
  pivot_longer(cols = V20:V1, 
               names_to = 'site', 
               values_to = 'score') %>%
  mutate(site = as.numeric(str_remove(site, "V"))) %>%
  filter(sum > 60) %>% 
  unique() %>%
  ggplot() +
  geom_boxplot(aes(x = -site, 
                   y = score, 
                   group = site),
               fill = 'transparent') +
  facet_wrap(~pset)
```


```{r}
primer_threshold(fw = "output/Primer_F2_248_267_all.csv",
                 rw = "output/Primer_R2_587_606_all.csv", 
                 threshold = 120)
```


```{r}
Primer_R2_587_606_all %>%
  drop_na(sum) %>%
  select(Template, sum) %>% 
  separate(Template, into = c("Species", "ID"), sep = "_(?=[^_]+$)") %>%
  select(-ID) %>%
  group_by(Species) %>%
  summarize_all(c(mean = mean, min = min, max = max, sd = sd)) %>% View() #On average 67% of amphib species amplifiable. 
```

```{r}
Primer_F2_248_267_all %>%
  drop_na(sum) %>%
  select(Template, sum) %>% 
  separate(Template, into = c("Species", "ID"), sep = "_(?=[^_]+$)") %>%
  select(-ID) %>%
  group_by(Species) %>%
  summarize_all(c(mean = mean, min = min, max = max, sd = sd)) %>% View() #On average 67% of amphib species amplifiable. 
```

```{r}
left_join(Primer_F2_248_267, Primer_R2_587_606, by = "Template",
          suffix = c("for", "rev")) %>%
  drop_na(sumrev, sumfor) %>% 
  mutate(sum = sumrev + sumfor) %>% 
  separate(Template, into = c("Species", "ID"), sep = "_(?=[^_]+$)") %>% View
  filter(sum > 120) %>% 
  select(Species) %>%
  unique() %>% View()
```




